...
function addStudent(input) {
    var jsonData = {};
    jsonData.id = input[0];
    jsonData.name = input[1];
    jsonData.pic = input[2];
    studentData.students.push(jsonData);
}

function start() {
    var tables = document.querySelectorAll('.TR0, .TR1');
    for (i of tables) {
        addStudent([
            i.childNodes[1].textContent,
            i.childNodes[3].textContent.trim(),
            i.childNodes[3].getElementsByTagName('a')[0].getAttribute('onmouseover').match(/"(.*?)"/)[1]
        ]);
    }
    var blob = new Blob([JSON.stringify(studentData, null, 4)], {
